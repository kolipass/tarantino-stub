...
public class HttpApiExceptionStrategy extends AbstractErrorHandlerStrategy {
    ...
    private void blockTokenException(@NonNull HttpApiException exception) {
        if (listener != null) listener.onHandleErrorMessage(getErrorMessage(exception));
        intentStarter.logout();
    }

    private void tokenException(@NonNull HttpApiException exception) {
        if (listener != null) listener.onHandleErrorMessage(getErrorMessage(exception));
        intentStarter.logout();
    }

    private void appVersionException(@NonNull HttpApiException exception) {
        if (listener != null) listener.onHandleErrorMessage(getErrorMessage(exception));
        intentStarter.logout();
        intentStarter.updateApp();
    }

    protected void unauthorizedException(@NonNull HttpApiException exception) {
        if (listener != null) listener.onHandleErrorMessage(getErrorMessage(exception));
        intentStarter.logout();
    }

    private String getErrorMessage(@NonNull HttpApiException e) {
        String message = "";
    ...
    private String getUserMessage(@NonNull ErrorMobiApiAnswer apiAnswer) {
