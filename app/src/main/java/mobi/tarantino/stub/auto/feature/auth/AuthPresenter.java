...
public class AuthPresenter extends MvpLceMultiRxPresenter<AuthView> {
    ...
    public void sendPhone(@NonNull String phone) {
        unsubscribe();
        setPhone(phone);
        if (getView() != null && isViewAttached()) {
            sendPhoneProgress();
    ...
    public Subscriber<AuthTokenMobiApiAnswer> getSendCodeSubscriber() {
        return new Subscriber<AuthTokenMobiApiAnswer>() {
            ...
            @Override
            public void onNext(@NonNull AuthTokenMobiApiAnswer authTokenMobiApiAnswer) {
    ...
    private void sendCodeSuccess(@NonNull AuthTokenMobiApiAnswer authTokenMobiApiAnswer) {
