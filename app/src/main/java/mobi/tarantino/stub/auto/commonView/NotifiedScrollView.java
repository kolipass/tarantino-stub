...
public class NotifiedScrollView extends ScrollView {
    protected Set<Integer> _shownViewsIndices = new HashSet<Integer>();
    ...
    private void notifyListener(View view, int index, boolean becameVisible) {
        if (listener != null && view.getVisibility() != GONE) {
            listener.onChildViewVisibilityChanged(index, view, becameVisible);
        }
    }

    private boolean isViewInVisibleButNonNotified(int parentTop, int parentBottom, View v, int i) {
        return isViewInVisibleArea(parentTop, parentBottom, v) && !_shownViewsIndices.contains(i);
    }

    protected boolean isViewInVisibleArea(int parentTop, int parentBottom, View view) {
